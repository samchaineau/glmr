...
#' @param ChosenFamily The statistical family of the model.
...
glm_all <-function(z, ChosenFamily, data, pval){
  if(isFALSE(pval)){return <- lapply(z, function(z){glm(z, family = ChosenFamily, data=data)})}else{
    Models_list <- lapply(z, function(z){glm(z, family = ChosenFamily, data=data)})
